<template>
  <div class="project-list-container">
      ...
      <el-row :gutter="20">
        <el-col :xs="24" :sm="12" :md="8" :lg="6" v-for="project in projects" :key="project.id" class="project-col">
          <el-card :body-style="{ padding: '0px' }" class="project-item-card" shadow="hover">
            <!-- 点击图片打开详情 -->
            <div class="image-wrapper" @click="openDetailDialog(project)" style="cursor: pointer;">
              <!-- 封面图 -->
              <img 
              ...
              <div 
                class="subscribe-btn" 
                @click.stop="toggleSubscription(project)" 
                :title="isSubscribed(project.id) ? '取消订阅' : '添加订阅'"
              >
                <el-icon :size="18" :color="isSubscribed(project.id) ? '#F56C6C' : '#FFFFFF'">
                  <StarFilled v-if="isSubscribed(project.id)" />
                  <Star v-else />
                </el-icon>
              </div>
            ...
            <div class="project-content">
              <!-- 点击标题打开详情 -->
              <h3 class="project-title" @click="openDetailDialog(project)">{{ project.title }}</h3>
    ...
    <el-dialog v-model="detailDialogVisible" title="项目详情" width="600px" destroy-on-close>
      <div v-if="currentDetailProject" class="project-detail-box">
        <img :src="resolveImageUrl(currentDetailProject.coverImage)" class="detail-cover" @error="handleImageError" />
        <h2 class="detail-title">{{ currentDetailProject.title }}</h2>
        <div class="detail-tags">
          <el-tag size="small" type="info" plain>{{ currentDetailProject.category }}</el-tag>
          <el-tag :type="currentDetailProject.projectStatus === 1 ? 'success' : 'info'" effect="dark" style="margin-left: 10px;">
            {{ currentDetailProject.projectStatus === 1 ? '募集中' : '已结束' }}
          </el-tag>
        </div>
        ...
        <div class="detail-progress">
          ...
          <el-progress 
            :percentage="calculatePercent(currentDetailProject)" 
            :status="currentDetailProject.projectStatus === 1 ? '' : 'exception'" 
            :stroke-width="10"
            :show-text="false" 
          />
          <div style="margin-top: 10px; text-align: right; color: #e74c3c; font-weight: bold; font-size: 16px;">
            已筹善款：¥{{ currentDetailProject.currentAmount || 0 }}
          </div>
        </div>

        <el-divider>项目详细介绍</el-divider>
        <div class="detail-content">
          {{ currentDetailProject.content }}
        </div>
      </div>
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button 
            type="primary" 
            :disabled="currentDetailProject?.projectStatus !== 1" 
            @click="handleDonateFromDetail"
          >
            我要捐赠
          </el-button>
        </span>
...
const currentDetailProject = ref<any>(null)
...
// 在详情弹窗中点击捐赠
const handleDonateFromDetail = () => {
  if (currentDetailProject.value) {
    detailDialogVisible.value = false // 先关闭详情弹窗
    openDonateDialog(currentDetailProject.value) // 打开捐赠弹窗
  }
}
...
const loadUserSubscriptions = async () => {
  ...
  try {
    ...
    if (res.code === 200 && res.data) {
      const ids = res.data.map((sub: any) => sub.projectId)
      subscribedIds.value = new Set(ids)
    }
  } catch (e) {
    console.error('加载用户订阅状态失败', e)
  }
}

// 检查某个项目是否被订阅
const isSubscribed = (projectId: number) => {
  return subscribedIds.value.has(projectId)
}

// 切换订阅状态逻辑
const toggleSubscription = async (project: any) => {
  const userId = authStore.userInfo?.id
  if (!userId) {
    ElMessage.warning('请先登录后再进行订阅操作')
    return
  }
  ...
  try {
    if (isSubscribed(projectId)) {
      ...
      if (res.code === 200) {
        subscribedIds.value.delete(projectId)
        ElMessage.success(`已取消关注《${project.title}》`)
      } else {
        ElMessage.error(res.msg || '取消订阅失败')
      }
    } else {
      const res: any = await subscribeProject(userId, projectId)
      if (res.code === 200) {
        subscribedIds.value.add(projectId)
        ElMessage.success(`成功订阅《${project.title}》`)
      } else {
...
.project-title { margin: 0 0 10px 0; font-size: 18px; font-weight: 600; color: #2c3e50; height: 50px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; cursor: pointer; transition: color 0.3s; }
.project-title:hover { color: #409EFF; }
